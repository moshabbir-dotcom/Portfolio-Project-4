...
* When attempting to test the email functionality the website was crashing which was due to the external website not being configured to the website which from research online leads me to belive was related to firewall/security settings set by the learning provider. In order to allow this functionality to work for marking it is required  run <em>python -m smtpd -n -c DebuggingServer localhost:1025</em> in a new terminal while port 8000 is running to test functionality. This will be configured to work with the site owners email on the non graded production site.
